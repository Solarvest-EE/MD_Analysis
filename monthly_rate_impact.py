"""
Monthly Rate Impact Analysis Module

This module provides comprehensive monthly rate impact analysis functionality
comparing old TNB tariffs vs new RP4 tariffs on a monthly basis.

Features:
- Data interval auto-detection
- Old vs New tariff comparison
- RP4 tariff integration with holiday-aware peak/off-peak logic
...
def show():
    """Main function to display the Monthly Rate Impact Analysis interface"""
    
    # Import required modules for Monthly Rate Impact Analysis
    try:
        ...
        # Get AFA rate from session state (configured in sidebar)
        global_afa_rate = st.session_state.get('global_afa_rate', 0.0) / 100  # Convert from cents to RM
        ...
        st.error("Please ensure all required modules are available in the project structure.")